...
public class HistoryCleanupJob extends QuartzJob {

    private final Period maxHistory;

    private final List<CrudDatabaseHandler<?, ?>> tableHandlers;

    public HistoryCleanupJob(Period maxHistory, PrincipalsHandler principalsHandler, List<CrudDatabaseHandler<?, ?>> tableHandlers) {
        super(principalsHandler);
    ...
    protected void setPrincipalsOnDbHandlers(Principals principals) {
        tableHandlers.forEach(v -> v.setPrincipals(principals));
    }
}
