...
public class CaConsistencyChecker extends QuartzJob {

    private static final Logger LOG = LogManager.getLogger(CaConsistencyChecker.class);
    ...
    public CaConsistencyChecker(
            AuthAdmin authAdmin,
            PrincipalsHandler dbHandler,
            UserDeviceHandler userDeviceHandler,
            FoodItemHandler foodItemHandler,
            AuthenticationManager authenticationManager
    ) {
        super(dbHandler, authenticationManager);
    ...
    private void removeRevokedDevicesFromDb(Set<Principals> caPrincipals, Set<Principals> dbPrincipals) {
        ...
        Principals currentUser = getPrincipals();

        for (Principals p : toRemove) {
            StatusCode result = foodItemHandler.transferFoodItems(p.toDevice(), currentUser.toDevice())
