...
public class UnitManager extends BusinessObject<UnitRecord, Unit>
        implements BusinessGettable<UnitRecord, Unit>,
                   ...
                   BusinessDeletable<UnitForDeletion, Unit> {

    private final UnitHandler dbHandler;

    public UnitManager(UnitHandler dbHandler) {
        super(dbHandler);
        this.dbHandler = dbHandler;
    }

    public StatusCode rename(UnitForRenaming unit) {
        return runOperation(() -> dbHandler.rename(unit));
    }

    public StatusCode delete(UnitForDeletion unit) {
        return runOperation(() -> dbHandler.delete(unit));
    }
}
