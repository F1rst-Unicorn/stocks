...
if [[ "$RESOURCES" == "" ]] ; then
    RESOURCES=""
    echo "Resource directory has not been defined"
    exit 1
fi

if [[ "$SERVER" == "" ]] ; then
    SERVER=""
    echo "Server name has not been defined"
    exit 1
fi
...
addDevice() {
    TICKET=$(curl -sS ${CURLARGS} -XPUT https://${SERVER}:10912/device \
            --header 'content-type: application/json' \
            --data '{"id":0,"name":"Device","userId":1}')
}
addEan() {
    curl -sS ${CURLARGS} -XPUT https://${SERVER}:10912/ean \
            --header 'content-type: application/json' \
