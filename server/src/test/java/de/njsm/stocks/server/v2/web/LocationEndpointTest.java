...
public class LocationEndpointTest {
    ...
    public void getLocationReturnsList() {
        ...
        when(businessLayer.get(Instant.EPOCH, Instant.EPOCH)).thenReturn(Validation.success(data));

        var actual = uut.get(InstantSerialiser.serialize(Instant.EPOCH), InstantSerialiser.serialize(Instant.EPOCH));

        assertEquals(SUCCESS, actual.getStatus());
        assertEquals(data, ((ListResponse<Location>) actual).getData());
        verify(businessLayer).get(Instant.EPOCH, Instant.EPOCH);
    }
    ...
    public void renameLocationWorks() {
        ...
        Response response = uut.renameLocation(data.id(), data.version(), data.name());
