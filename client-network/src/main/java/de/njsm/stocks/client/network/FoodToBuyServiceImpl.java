...
import de.njsm.stocks.client.business.FoodToBuyService;
...
class FoodToBuyServiceImpl extends ServiceCommand<FoodForBuying> implements FoodToBuyService {
    ...
    @Override
    public void editToBuy(FoodForBuying food) {
        perform(food);
    }

    @Override
    Call<? extends Response> buildCall(FoodForBuying input) {
        return api.editFoodToBuy(input.id(), input.version(), input.toBuy() ? 1 : 0);
    }
}
