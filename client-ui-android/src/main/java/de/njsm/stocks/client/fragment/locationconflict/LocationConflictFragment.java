...
public class LocationConflictFragment extends BottomToolbarFragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        locationViewModel.getLocationEditConflict(errorId).observe(getViewLifecycleOwner(), v -> {
            ...
            if (v.name().needsHandling()) {
                form.showNameConflict();
                form.setOriginalName(v.name().original());
                form.setRemoteName(v.name().remote());
                form.setLocalName(v.name().local());
            } else {
                form.hideName();
            }
    ...
    @Inject
    public void setLocationConflictNavigator(LocationConflictNavigator locationConflictNavigator) {
