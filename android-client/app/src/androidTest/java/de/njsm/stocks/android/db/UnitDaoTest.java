...
public class UnitDaoTest extends DbTestCase {
    ...
    public void insertionWorks() {
        Unit data = new Unit(0, 1, Instant.EPOCH, Config.DATABASE_INFINITY, Instant.EPOCH, Config.DATABASE_INFINITY, 0, 1, "name", "abbreviation");
    ...
    public void insertionWithConflictReplaces() {
        Unit dataToBeTerminated = new Unit(0, 1, Instant.EPOCH, Config.DATABASE_INFINITY, Instant.EPOCH, Config.DATABASE_INFINITY, 0, 1, "name", "abbreviation");
        unitDao.insert(new Unit[]{dataToBeTerminated});
        Instant now = Instant.now();
        dataToBeTerminated.transactionTimeEnd = now;
        Unit terminatedData = new Unit(0, 1, Instant.EPOCH, now, Instant.EPOCH, Config.DATABASE_INFINITY, 0, 1, "name", "abbreviation");
