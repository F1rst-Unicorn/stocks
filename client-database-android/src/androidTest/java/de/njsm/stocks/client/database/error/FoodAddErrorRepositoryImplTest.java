...
import de.njsm.stocks.client.business.entities.FoodAddErrorDetails;
...
public class FoodAddErrorRepositoryImplTest extends AbstractErrorRepositoryImplTest {
    ...
    ErrorDetails recordError(StatusCodeException e) {
        ...
        FoodAddForm form = FoodAddForm.create("Banana", true, Period.ZERO, location.id(), scaledUnit.id(), "they are yellow");
        errorRecorder.recordFoodAddError(e, form);
        return FoodAddErrorDetails.create(
                form.name(),
                form.toBuy(),
                form.expirationOffset(),
                form.location().orElse(null),
                form.storeUnit(),
                form.description(),
                location.name(),
    ...
    List<?> getErrorDetails() {
        return stocksDatabase.errorDao().getFoodAdds();
    }
}
