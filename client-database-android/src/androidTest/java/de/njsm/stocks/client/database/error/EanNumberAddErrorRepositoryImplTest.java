...
import de.njsm.stocks.client.business.entities.EanNumberAddErrorDetails;
...
public class EanNumberAddErrorRepositoryImplTest extends AbstractErrorRepositoryImplTest {
    ...
    ErrorDetails recordError(StatusCodeException e) {
        ...
        EanNumberAddForm form = EanNumberAddForm.create(food::id, "123");
        errorRecorder.recordEanNumberAddError(e, form);
        return EanNumberAddErrorDetails.create(
                food.id(),
                food.name(),
                form.eanNumber()
        );
    ...
    List<?> getErrorDetails() {
        return stocksDatabase.errorDao().getEanNumberAdds();
    }
}
