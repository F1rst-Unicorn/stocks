...
\begin{enumerate}
    ...
    \item Try to load $F$ at the latest valid time start before "now" and
            transaction time "infinity". This has to be done if $F$ has been current-deleted
            between recording the error and loading the error.
    \item Try to load $F$ at the latest valid time start before "now" and
            transaction time $t$. This has to be done if $F$ has been sequenced-deleted for
            the entire valid time between recording the error and loading the error.
\end{enumerate}
